import { FilterParams, OverviewData, TimeseriesDataPoint, RankingUF, ForecastDataPoint, PrevisaoResponse, TopVacinaDataPoint, SazonalidadeDataPoint, DetalhesGeograficosDataPoint } from "@/types/apiTypes";
...
class APIClient {
  ...
  const url = this.buildURL("/api/timeseries", params);
    ...
    // - { data, doses_distribuidas } (legacy)
    // - { periodo, doses } (new)
    // Normalize common wrapper shapes defensively.
    if (Array.isArray(payload)) {
      return payload.map((p: unknown) => {
        const obj = p as Record<string, unknown>;
        const data = String(obj["periodo"] ?? obj["data"] ?? "");
        const doses = Number(obj["doses"] ?? obj["doses_distribuidas"] ?? 0);
        return { data, doses_distribuidas: doses } as TimeseriesDataPoint;
      });
    }
    if (payload && typeof payload === "object") {
      const obj = payload as Record<string, unknown>;
      if (Array.isArray(obj["data"])) {
        return (obj["data"] as Array<unknown>).map((p) => {
          ...
          const data = String(item["periodo"] ?? item["data"] ?? "");
          const doses = Number(item["doses"] ?? item["doses_distribuidas"] ?? 0);
          return { data, doses_distribuidas: doses } as TimeseriesDataPoint;
        });
      }
  ...
  async getTopVacinas(params?: FilterParams): Promise<TopVacinaDataPoint[]> {
    const url = this.buildURL("/api/top-vacinas", params);
    const response = await fetch(url);
    if (!response.ok) {
      const txt = await response.text();
      const err = new Error(`Erro ao buscar top vacinas: ${response.status} ${txt}`) as Error & { status?: number; body?: unknown };
      err.status = response.status;
      err.body = txt;
      throw err;
    }
    ...
    if (Array.isArray(payload)) {
      return payload.map((p: unknown) => {
        const obj = p as Record<string, unknown>;
        return {
          tx_insumo: String(obj["tx_insumo"] ?? ""),
          qtde: Number(obj["qtde"] ?? 0),
        } as TopVacinaDataPoint;
      });
    }
  ...
  async getSazonalidade(params?: Pick<FilterParams, "ano" | "uf" | "fabricante">): Promise<SazonalidadeDataPoint[]> {
    const url = this.buildURL("/api/sazonalidade", params as FilterParams | undefined);
    const response = await fetch(url);
    if (!response.ok) {
      const txt = await response.text();
      const err = new Error(`Erro ao buscar sazonalidade: ${response.status} ${txt}`) as Error & { status?: number; body?: unknown };
      err.status = response.status;
      err.body = txt;
      throw err;
    }
  ...
  async getDetalhesGeograficos(params?: Pick<FilterParams, "ano" | "mes" | "fabricante">): Promise<DetalhesGeograficosDataPoint[]> {
    const url = this.buildURL("/api/detalhes-geograficos", params as FilterParams | undefined);
    const response = await fetch(url);
    if (!response.ok) {
      const txt = await response.text();
      const err = new Error(`Erro ao buscar detalhes geograficos: ${response.status} ${txt}`) as Error & { status?: number; body?: unknown };
      err.status = response.status;
      err.body = txt;
      throw err;
    }
    ...
    if (Array.isArray(payload)) {
      return payload.map((p: unknown) => {
        const obj = p as Record<string, unknown>;
        return {
          tx_sigla: String(obj["tx_sigla"] ?? ""),
          tx_insumo: String(obj["tx_insumo"] ?? ""),
          qtde: Number(obj["qtde"] ?? 0),
        } as DetalhesGeograficosDataPoint;
      });
    }
