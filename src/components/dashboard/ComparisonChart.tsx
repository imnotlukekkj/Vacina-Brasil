...
const ComparisonChart = ({ data, loading, embedded = false }: ComparisonChartProps) => {
  ...
  const growthPercentage = useMemo(() => {
    const findByTipoOrAno = (kind: "historico" | "projecao") => {
      return barData.find((entry) => {
        const tipo = String(entry.tipo || "").toLowerCase();
        const ano = Number(entry.ano);
        if (kind === "historico") return tipo.includes("histor") || ano === 2024;
        return tipo.includes("proje") || ano === 2025;
      });
    };

    const historico = findByTipoOrAno("historico")?.quantidade;
    const projecao = findByTipoOrAno("projecao")?.quantidade;
    const histNum = historico === null || historico === undefined ? NaN : Number(historico);
    const projNum = projecao === null || projecao === undefined ? NaN : Number(projecao);

    if (!Number.isFinite(histNum) || !Number.isFinite(projNum) || histNum <= 0) return null;
    return ((projNum - histNum) / histNum) * 100;
  }, [barData]);

  const growthLabel = useMemo(() => {
    if (growthPercentage === null || !Number.isFinite(growthPercentage)) return null;
