...
from .repositories.supabase_repo import get_supabase_client, rpc_timeseries_aggregated
...
async def _fetch_timeseries_from_db(table: str, filters: Dict[str, Optional[str]]) -> List[Dict[str, Any]]:
    """Busca série temporal agregada por ano e mês direto no banco.
    Quando `ano` é informado, o agrupamento permanece por `ano, mes` para
    preservar a evolução mensal do ano selecionado."""
    global db_pool
    if not db_pool:
        return []
    ...
    query = (
        f"SELECT ano, mes, SUM(qtde) AS doses "
        f"FROM {table}"
    )
    if clauses:
        query += " WHERE " + " AND ".join(clauses)
...
async def _fetch_timeseries_from_supabase(table: str, filters: Dict[str, Optional[str]]) -> List[Dict[str, Any]]:
    """Busca série temporal já agregada no Supabase via RPC.
    Evita transferir milhares de linhas cruas para agregação em memória."""
    supabase_url = os.getenv("SUPABASE_URL")
    supabase_key = os.getenv("SUPABASE_SERVICE_ROLE_KEY")
    if not supabase_url or not supabase_key:
        return []
    client = get_supabase_client()
    rows, rpc_raw = await rpc_timeseries_aggregated(
        client,
        supabase_url,
        supabase_key,
        {
            ...
            "mes": filters.get("mes"),
            "uf": filters.get("uf"),
            "fabricante": filters.get("fabricante"),
        },
        {
            "_ano": int(filters.get("ano")) if filters.get("ano") else None,
            "_mes": int(filters.get("mes")) if filters.get("mes") else None,
            "_uf": filters.get("uf"),
            "_fabricante": filters.get("fabricante"),
        },
    )
    if rows is None:
        logger.warning("Supabase timeseries RPC failed: %s", rpc_raw)
        return []
    return rows
...
async def timeseries(ano: Optional[str] = None, mes: Optional[str] = None, uf: Optional[str] = None, fabricante: Optional[str] = None):
    filters = {"ano": ano, "mes": mes, "uf": uf, "fabricante": fabricante}

    # Prefer SQL aggregation when DB connection is available.
    # Even with `ano` filter, grouping stays on ano+mes to preserve monthly evolution.
    if db_pool:
        agg_rows = await _fetch_timeseries_from_db(DATA_TABLE, filters)
        result = []
        for r in agg_rows:
    ...
    # Supabase REST path: use paginated fetch to avoid truncation to first page.
    if os.getenv("SUPABASE_URL") and os.getenv("SUPABASE_SERVICE_ROLE_KEY"):
        agg_rows = await _fetch_timeseries_from_supabase(DATA_TABLE, filters)
        result = []
        for r in agg_rows:
            ano_val = int(r.get("ano") or 0)
            mes_val = int(r.get("mes") or 0)
    ...
    rows = await _fetch_rows(DATA_TABLE, filters)
    for r in rows:
        r.setdefault("tx_insumo_norm", normalizer.normalize_insumo(r.get("tx_insumo")))
        r.setdefault("tx_sigla_norm", normalizer.normalize_sigla(r.get("tx_sigla")))
